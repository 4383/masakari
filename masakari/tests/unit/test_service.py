...
class TestWSGIService(base.NoDBTestCase):
    ...
    def test_reset_pool_size_to_default(self):
        # Mock DynamicThreadPoolExecutor to have testable size attribute
        with mock.patch('masakari.api.wsgi.DynamicThreadPoolExecutor') as \
            ...
            initial_pool.size = CONF.wsgi.default_pool_size

            reset_pool = mock.Mock()
            reset_pool.size = CONF.wsgi.default_pool_size
            ...
            # Verify that reset created a new pool with default size
            self.assertEqual(test_service.server._pool.size,
                           CONF.wsgi.default_pool_size)
